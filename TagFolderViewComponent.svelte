<script lang="ts">
  ...
  import { type ViewItem, type TagFolderSettings, type TREE_TYPE, type TagFolderListState } from './types';
  ...
  let headerTitle = $derived(
    title == '' ? `${viewType == 'tags' ? 'Tags' : 'Links'}: ${vaultName}` : `Items: ${title}`
  );
  const viewItems = $derived.by(() => {
    ...
    for (const tag of lowerTags) {
      items = items.filter((e) => e.tags.some((e) => (e.toLowerCase() + '/').startsWith(tag)));
    }
    ...
    if (!archiveTags.contains(firstLevel)) {
      items = items.filter((item) => !item.tags.some((e) => archiveTags.contains(e.toLowerCase())));
    }
    return items;
  });
