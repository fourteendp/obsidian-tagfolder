<script lang="ts">
  ...
  import { type TREE_TYPE, type TagFolderSettings, type ViewItem } from 'types';
  import {
    renderSpecialTag,
    escapeStringToHTML,
  ...
  import { currentFile, pluginInstance, selectedTags, tagFolderSetting, tagInfo, v2expandedTags } from './store';
  ...
  interface Props {
    ...
    headerTitle?: string;
    // -- Callbacks --
    showMenu: (evt: MouseEvent, trail: string[], targetTag?: string, targetItems?: ViewItem[]) => void;
    openFile: (path: string, specialKey: boolean) => void;
    hoverPreview: (e: MouseEvent, path: string) => void;
    openScrollView: (leaf: undefined, title: string, tagPath: string, files: string[]) => Promise<void>;
  }
  ...
  const expandLimit = $derived(
    !_setting.expandLimit ? 0 : viewType == 'links' ? _setting.expandLimit + 1 : _setting.expandLimit
  );
  ...
  function handleOpenScroll(e: MouseEvent, trails: string[], filePaths: string[]) {
    if (viewType == 'tags') {
      openScrollView(undefined, '', joinPartialPath(removeIntermediatePath(trails)).join(', '), filePaths);
  ...
  // --> Self information
  const filename = $derived(viewType == 'tags' ? '' : thisName.substring(thisName.indexOf(':') + 1));
  const thisInfo: ViewItem | undefined = $derived(viewType != 'links' ? undefined : getViewItemFromPath(thisName));
  const thisLinks: string[] = $derived(viewType != 'links' ? [] : (thisInfo?.links ?? []).map((e) => `${e}`));
  ...
  const inMiddleOfTagHierarchy = $derived(trail.length >= 1 && trail[trail.length - 1].endsWith('/'));
  const previousTrail = $derived(inMiddleOfTagHierarchy ? trail[trail.length - 1] : '');
  const lastTrailTagLC = $derived(trimTrailingSlash(previousTrail).toLowerCase());
  ...
  const tagsAllCI = $derived(uniqueCaseIntensive(_items.flatMap((e) => e.tags)));
  // And them in lower case
  const tagsAllLower = $derived(tagsAllCI.map((e) => e.toLowerCase()));

  // Is this tag in dedicated tag?
  const isInDedicatedTag = $derived(inMiddleOfTagHierarchy && !tagsAllLower.contains(lastTrailTagLC));
  ...
  const tagsExceptAlreadyShown = $derived(
    viewType != 'tags'
      ...
      : displayTagCandidates.filter((tag) =>
          trail.every((trail) => trimTrailingSlash(tag.toLowerCase()) !== trimTrailingSlash(trail.toLowerCase()))
        )
  );
  ...
  const escapedPreviousTrail = $derived(!isMixedDedicatedTag ? previousTrail : previousTrail.split('/').join('*'));
  const sparseIntermediateTags = $derived.by(() => {
    const t1 = !isInDedicatedTag
      ? passedTagWithoutThis
      : passedTagWithoutThis.filter((e) => (e + '/').startsWith(previousTrail));
  ...
  const { filteredTags, isSuppressibleLevel } = $derived.by(() => {
    ...
    if (!isSuppressibleLevel) {
      ...
      if (viewType == 'tags') {
        tagsOnNextLevel = uniqueCaseIntensive(
          existTags.map((e) => {
            ...
            let idx2 = idx;
            while (removeItems.some((e) => e.startsWith(piece.toLowerCase()))) {
      ...
      existTagsFiltered1 = tagsOnNextLevel.filter((tag) =>
        ...
        trailShortest.every((trail) => trimTrailingSlash(tag.toLowerCase()) !== trimTrailingSlash(trail.toLowerCase()))
      );
    }
  ...
  const { tags, leftOverItemsSrc } = $derived.by(() => {
    ...
    if (previousTrail.endsWith('/')) {
      ...
      for (const tag of filteredTags) {
        if (!filteredTags.map((e) => e.toLowerCase()).contains((previousTrail + tag).toLowerCase())) {
          existTagsFiltered4.push(tag);
        }
      }
      tags = uniqueCaseIntensive(removeIntermediatePath(existTagsFiltered4));
    } else {
  ...
  const leftOverItemsUnsorted = $derived.by(() => {
    ...
    if (isRoot && isMainTree && !isSuppressibleLevel) {
      ...
      if (_setting.expandUntaggedToRoot) {
        return _items.filter((e) => e.tags.contains('_untagged') || e.tags.contains('_unlinked'));
      } else {
        return [] as ViewItem[];
      }
    }
  ...
  const leftOverItems = $derived(
    _setting.sortExactFirst ? performSortExactFirst(_items, children, leftOverItemsUnsorted) : leftOverItemsUnsorted
  );
  ...
  const tagsDisp = $derived(
    isSuppressibleLevel && isInDedicatedTag
      ? [[...tagNameDisp, ...suppressLevels.flatMap((e) => e.split('/').map((e) => renderSpecialTag(e)))]]
      : isSuppressibleLevel
        ? [tagNameDisp, ...suppressLevels.map((e) => e.split('/').map((e) => renderSpecialTag(e)))]
        : [tagNameDisp]
  );
          ...
          .map(
            (e) =>
              `<span class="tagfolder-tag tag-tag${classKey}">${e
                .map((ee) => `<span class="tf-tag-each">${escapeStringToHTML(ee)}</span>`)
                .join('')}</span>`
          )
          .join('')
      : ''
  );
  const itemCount = $derived(viewType == 'tags' ? (_items?.length ?? 0) : tags.length + leftOverItems.length);
...
>
  ...
  {:else}
    ...
    >
      ...
      <div class="tree-item-inner nav-folder-title-content lsl-f" onclick={handleOpenItem}>
        {#if isFolderVisible}
          <div class="tagfolder-titletagname" {draggable} ondragstart={dragStartName}>
            {@html tagsDispHtml}
          </div>
        {:else}
        ...
        >
          <span class="itemscount" {draggable} ondragstart={dragStartFiles}>{itemCount}</span>
        </div>
      </div>
    </OnDemandRender>
  {/if}
      ...
      {#each leftOverItemsDisp as items}
        {#each items as item}
          <TreeItemItemComponent
            {item}
            {openFile}
            trail={isRoot ? [...trail] : [...trail, ...suppressLevels]}
            {showMenu}
            {hoverPreview}
          />
        {/each}
