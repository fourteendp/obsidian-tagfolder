<script lang="ts">
  ...
  import { renderSpecialTag, trimSlash, escapeStringToHTML, getExtraTags, uniqueCaseIntensive } from './util';
  ...
  let { item, trail, openFile, showMenu, hoverPreview }: Props = $props();
  ...
  const tagsLeft = $derived(
    isItemVisible
      ? uniqueCaseIntensive(
          [...getExtraTags(item.tags, [...trail], _setting.reduceNestedParent), ...item.extraTags]
  ...
  const extraTagsHtml = $derived(
    `${tagsLeft.map((e) => `<span class="tf-tag">${escapeStringToHTML(e)}</span>`).join('')}`
  );
